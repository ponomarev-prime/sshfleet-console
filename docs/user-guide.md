...
- `SOURCES` — узкая навигация. `All available` всегда первый, затем локальные,
  ...
  количеством узлов; происхождение host при этом не меняется.
...
| Sources | `Enter` | применить source/group и перейти в Hosts |
...
до закрытия и доступна через `Alt+номер`. Preview terminal показывает
...
`exit` или `~.` в начале строки. `Alt+1…9` выбирает слот напрямую (`1` — Fleet, `2` —
первая сессия). Дополнительно принимается `Ctrl+1…9`, когда terminal emulator
передаёт modified-number и не резервирует сочетание. Если прямое сочетание
занято внешним терминалом, `Ctrl+N`/`Ctrl+P` циклически переключают вкладки, а
`Ctrl+G` возвращает Fleet.
